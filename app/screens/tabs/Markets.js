...
const Markets: () => Node = () =>{
  ...
  const Images = [
    ...
    { uri: "https://www.caesar-resort.com/files-sbbasic/ba_caesarresortspa_es/caesar-resort-spa-neighborhod-02.jpg?undefined" },
    ...
    <ScrollView>
      ...
      //   NavigationService.navigate('ModalEachProduct')
        ...
        <View style={{ flex: 0.6 }}>
          ...
          <Text style={{ flex: 0.2, fontSize: 15, fontWeight: 'bold', color: '#6200EE' }}>{item.description}</Text>
          <Text style={{ flex: 0.2, fontSize: 12, }}>{item.time}</Text>
          <Text style={{ flex: 0.2, fontSize: 12, }}>{item.number}</Text>
        </View>
        {/*<View style={{  }}>*/}
